...
export default async function handleSetRewardCommand(
  _client: _Client,
  interaction: Interaction,
) {
  console.log("handleSetRewardCommand");
  ...
  console.log(
    `Setting reward for role ${roleName} (roleId: ${roleId}) with amount ${amount} and frequency ${frequency}`,
  );
  console.log(">>> guild", guild);
  console.log(">>> user", user);
  await interaction.reply({
    content: `✅ Reward set for role <@&${roleId}> to ${amount} tokens (${frequency})`,
    ephemeral: true,
  });
}
